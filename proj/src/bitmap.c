...
void drawBitmap(Bitmap* bmp, int x, int y, Alignment alignment) {
    ...
    for (i = 0; i < height; i++) {
        ...
        bufferStartPos = getDrawingBuffer();
...
//Draw the sprite without the ignored color defined

void drawSprite(Bitmap* bmp, int x, int y, Alignment alignment) {
    if (bmp == NULL)
        return;
    ...
    for (i = 0; i < height; i++) {
        ...
        for(j = 0; j < drawWidth * 2; j += 2){
                    if(imgStartPos[j]!=0x1f && imgStartPos[j+1] != BACKGROUND_COLOR){
...
void drawSpriteInverted(Bitmap* bmp, int x, int y, Alignment alignment) {
    if (bmp == NULL)
        return;
    ...
    if (x < 0) {
    	...
    	xCorrection = x + width - getHorResolution();
    	drawWidth -= xCorrection;
    }
